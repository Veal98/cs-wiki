...
const blockTex = (state, start, end, silent) => {
    ...
    token.block = true;
    token.content =
        ((firstLine === null || firstLine === void 0 ? void 0 : firstLine.trim()) ? `${firstLine}\n` : "") +
...
const katexInline = (tex, options) => {
    ...
    try {
        ...
        if (options.throwOnError)
            console.warn(error);
        return `<span class='katex-error' title='${(0, utils_1.escapeHtml)(error.toString())}'>${(0, utils_1.escapeHtml)(tex)}</span>`;
    }
};
const katexBlock = (tex, options) => {
    ...
    try {
        ...
        if (options.throwOnError)
            console.warn(error);
        return `<p class='katex-block katex-error' title='${(0, utils_1.escapeHtml)(error.toString())}'>${(0, utils_1.escapeHtml)(tex)}</p>`;
    }
};
exports.default = (md, options = { throwOnError: false }) => {
    ...
    md.renderer.rules.inlineTex = (tokens, idx) => katexInline(tokens[idx].content, options);
    md.renderer.rules.blockTex = (tokens, idx) => `${katexBlock(tokens[idx].content, options)}\n`;
};
