...
Sigmoid函数是一种非线性的S型函数，输出被限定在[0,1]之间，通常被用在神经网络中当作激活函数（Activation function）使用。Sigmoid 函数的表达式和曲线如下所示：
...
我们已经掌握了 Cost function 的表达式，接下来将使用梯度下降（Gradient Descent）算法来计算出合适的w和b值，从而最小化m个训练样本的Cost function，即J(w,b)。
...
上一部分介绍的是计算图的正向传播（Forward Propagation），下面我们来介绍其**反向传播（Back Propagation），即计算输出对输入的偏导数。**
...
现在，我们将对逻辑回归进行梯度计算。**对单个样本而言**，逻辑回归 Loss function 表达式如下：
