...
**项目概述**：使用朴素贝叶斯构建一个快速过滤器来屏蔽在线社区留言板上的侮辱性言论。<u>如果某条留言使用了负面或者侮辱性的语言，那么就将该留言标识为内容不当</u>。对此问题建立两个类别: 侮辱类和非侮辱类，使用 1 和 0 分别表示。
...
def setOfWords2Vec(vocabList, inputSet):
    ...
    # 遍历文档中的所有单词，如果出现了词汇表中的单词，则将词汇表中该单词的位置设为1（如果不理解下文有详细解释）
...
def trainNB0(trainMatrix, trainCategory):
    ...
    # 单词在每个文件中出现的次数列表
    p0Num = np.ones(numWords) # [0,0,0,.....] 非侮辱性单词在每个文件中出现的次数列表
    p1Num = np.ones(numWords) # [0,0,0,.....] 侮辱性单词出在每个文件中出现的次数列表

    # 单词在整个数据集出现的总数
    p0Denom = 2.0 # 0 非侮辱性词汇在整个数据集的出现总数
    p1Denom = 2.0 # 1 侮辱性词汇在整个数据集的出现总数
    
    for i in range(numTrainDocs):
        # 是否是侮辱性文件
        if trainCategory[i] == 1:
            # 如果是侮辱性文件，对侮辱性文件的向量进行相加，表示所有文件中各个侮辱性词汇出现的次数
            p1Num += trainMatrix[i] # [0,1,1,....] + [0,1,1,....] -> [0,2,2,...]
            # 对向量中的所有元素进行求和，表示所有文件中所有侮辱性词汇出现的次数之和
...
**pAb 是任意文档属于侮辱性文档的概率。p0V 表示词汇表中的每个单词在非侮辱性文档中出现的概率，p1V 表示词汇表中的每个单词在侮辱性文档中出现的概率。**
