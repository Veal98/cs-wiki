...
⭐ <u>一个马尔科夫奖励过程 MRP 中某一状态的**价值函数**为**从该状态开始**的马尔可夫链 MP 收获的期望</u>。价值函数的定义公式如下：
...
- 😀 对于状态比较少的 MRP，我们可以使用 **解析解** 来求解（即将贝尔曼等式转换成矩阵形式进行求解）：
...
- 😵 对于状态比较多的 MRP，我们可以使用 **迭代** 的方法来求解，后续文章会逐步讲解这些方法：
...
#### Ⅱ 行为价值函数 Action-Value Function / Q 函数

这里我们另外引入了一个 `Q 函数 / 状态动作值函数 (action-value function)`。

**行为价值函数 <img src="https://gitee.com/veal98/images/raw/master/img/20201021152758.png" style="zoom:50%;" /> 表示从状态 s 出发，已经采取了动作 a 之后，再使用策略 π（概率分布）所带来的累积奖赏**。
