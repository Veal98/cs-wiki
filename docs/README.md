...
- 2 - 分布式锁
- [2 - 分布式事务](系统设计/分布式/分布式理论基础/3-分布式事务.md)
...
> [!Danger]
> **Dubbo 与 Spring Cloud 并不是竞争关系**，**Dubbo** 作为成熟的 RPC 框架（分布式服务框架），其易用性、扩展性和健壮性已得到业界的认可。而 **Spring Cloud** 是一整个微服务生态，未来 **Dubbo** 将会作为 **Spring Cloud Alibaba** 的 RPC 组件，并与 **Spring Cloud** 原生的 **Feign** 以及 **RestTemplate** 进行无缝整合，实现“零”成本迁移。
>
> 💌 <u>[阿里巴巴中间件 独家解读：Dubbo Ecosystem - 从微服务框架到微服务生态](https://mp.weixin.qq.com/s/iNVctXw7tUGHhnF0hV84ww)</u>

- 2 - RPC 框架：**Apache Dubbo**
  - [2.1 - Dubbo 重要概念详解](系统设计/分布式/RPC/2-Dubbo重要概念详解.md)
  - 2.2 - SpringBoot + Dubbo + Zookeeper 搭建一个简单的分布式服务
