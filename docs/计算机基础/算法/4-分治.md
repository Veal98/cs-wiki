...
private void QuickSort(int[] arr, int left, int right) {
    ...
    QuickSort(arr, left, i - 1); // 对左半段排序
    QuickSort(arr, i + 1, right); // 对右半段排序
}
...
class Solution {
    ...
    private void quickSort (String[] strs, int left, int right) {
        ...
        String pivot = strs[left]; // 基准元素
        
        while (l < r) {
            while (l < r && 
                   strToInt(strs[r]+pivot) > strToInt(pivot+strs[r])) {
                r --;
            }
            while (l < r && 
                   strToInt(strs[l]+pivot) < strToInt(pivot+strs[l])) {
                l ++;
            }
